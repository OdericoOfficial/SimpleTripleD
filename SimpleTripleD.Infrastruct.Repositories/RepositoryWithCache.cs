...
namespace SimpleTripleD.Infrastruct.Repositories
{
        ...
        where TAggregateRoot : class, IAggregateRoot<TKey>
    {
        ...
        public async Task<TAggregateRoot> FirstAsync(string storeName, TKey id, CancellationToken cancellationToken = default)
        {
            ...
            var aggregateRoot = await _daprClient.GetStateAsync<TAggregateRoot>(storeName, storeKey, cancellationToken: cancellationToken).ConfigureAwait(false);
            if (aggregateRoot is not null)
                return aggregateRoot;
            aggregateRoot = await this.FirstAsync(id, cancellationToken).ConfigureAwait(false);
            await _daprClient.SaveStateAsync(storeName, storeKey, aggregateRoot, cancellationToken: cancellationToken).ConfigureAwait(false);
            return aggregateRoot;
        }
        ...
        {
            aggregateRoot = await InsertAsync(aggregateRoot, true, cancellationToken).ConfigureAwait(false);
            await _mediator.Publish(new TryDeleteCacheEventArgs<TKey>(aggregateRoot.Id, storeName), cancellationToken).ConfigureAwait(false);
            return aggregateRoot;
        }
        ...
        {
            aggregateRoot = await UpdateAsync(aggregateRoot, true, cancellationToken).ConfigureAwait(false);
            await _mediator.Publish(new TryDeleteCacheEventArgs<TKey>(aggregateRoot.Id, storeName), cancellationToken).ConfigureAwait(false);
            return aggregateRoot;
        }
        ...
        {
            await DeleteAsync(aggregateRoot, true, cancellationToken).ConfigureAwait(false);
            await _mediator.Publish(new TryDeleteCacheEventArgs<TKey>(aggregateRoot.Id, storeName), cancellationToken).ConfigureAwait(false);
        }

        public async Task DeleteAsync(string storeName, TKey id, CancellationToken cancellationToken = default)
        {
            await DeleteAsync(id, true, cancellationToken).ConfigureAwait(false);
            await _mediator.Publish(new TryDeleteCacheEventArgs<TKey>(id, storeName), cancellationToken).ConfigureAwait(false);
        }
    }
}